...
import { Candle, ProductToken } from 'constants/productTokens'
import useBalances from 'hooks/useBalances'
import useCndlTokenMarketData from 'hooks/useWlkrTokenMarketData'
...
const WlkrProductPage = (props: { title: string }) => {
  ...
  const { cndlBalance, wlkrBalancePolygon } = useBalances()
  const { cndlStreamingFee } = useStreamingFee()
  const { cndlTotalSupply } = useTokenSupply()
  const { cndlComponents: components } = useSetComponents()
  ...
  const getTokenBalance = () => {
    if (chainId) {
      if (chainId === MAINNET_CHAIN_DATA.chainId) return cndlBalance
      else if (chainId === POLYGON_CHAIN_DATA.chainId) return cndlBalancePolygon
    }
...
const StyledCndlIndexCalculationImage = styled.img`
  margin-bottom: 20px;
  width: 100%;
`
