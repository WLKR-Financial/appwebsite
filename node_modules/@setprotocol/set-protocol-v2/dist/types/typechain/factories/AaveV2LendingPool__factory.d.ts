import { Signer, ContractFactory, Overrides } from "ethers";
import { Provider, TransactionRequest } from "@ethersproject/providers";
import type { AaveV2LendingPool, AaveV2LendingPoolInterface } from "../AaveV2LendingPool";
export declare class AaveV2LendingPool__factory extends ContractFactory {
    constructor(linkLibraryAddresses: AaveV2LendingPoolLibraryAddresses, signer?: Signer);
    static linkBytecode(linkLibraryAddresses: AaveV2LendingPoolLibraryAddresses): string;
    deploy(overrides?: Overrides & {
        from?: string | Promise<string>;
    }): Promise<AaveV2LendingPool>;
    getDeployTransaction(overrides?: Overrides & {
        from?: string | Promise<string>;
    }): TransactionRequest;
    attach(address: string): AaveV2LendingPool;
    connect(signer: Signer): AaveV2LendingPool__factory;
    static readonly bytecode = "__$de8c0cf1a7d7c36c802af9a64fb9d86036$____$de8c0cf1a7d7c36c802af9a64fb9d86036$____$de8c0cf1a7d7c36c802af9a64fb9d86036$____$22cd43a9dda9ce44e9b92ba393b88fb9ac$____$de8c0cf1a7d7c36c802af9a64fb9d86036$____$de8c0cf1a7d7c36c802af9a64fb9d86036$____$de8c0cf1a7d7c36c802af9a64fb9d86036$____$de8c0cf1a7d7c36c802af9a64fb9d86036$__";
    static readonly abi: ({
        anonymous: boolean;
        inputs: {
            indexed: boolean;
            internalType: string;
            name: string;
            type: string;
        }[];
        name: string;
        type: string;
        outputs?: undefined;
        stateMutability?: undefined;
    } | {
        inputs: {
            internalType: string;
            name: string;
            type: string;
        }[];
        name: string;
        outputs: {
            components: ({
                components: {
                    internalType: string;
                    name: string;
                    type: string;
                }[];
                internalType: string;
                name: string;
                type: string;
            } | {
                internalType: string;
                name: string;
                type: string;
                components?: undefined;
            })[];
            internalType: string;
            name: string;
            type: string;
        }[];
        stateMutability: string;
        type: string;
        anonymous?: undefined;
    } | {
        inputs: {
            internalType: string;
            name: string;
            type: string;
        }[];
        name: string;
        outputs: {
            internalType: string;
            name: string;
            type: string;
        }[];
        stateMutability: string;
        type: string;
        anonymous?: undefined;
    })[];
    static createInterface(): AaveV2LendingPoolInterface;
    static connect(address: string, signerOrProvider: Signer | Provider): AaveV2LendingPool;
}
export interface AaveV2LendingPoolLibraryAddresses {
    ["__$de8c0cf1a7d7c36c802af9a64fb9d86036$__"]: string;
    ["__$22cd43a9dda9ce44e9b92ba393b88fb9ac$__"]: string;
}
